...
class DatasetImageMaskH5CardiacDyn(Dataset):
...
#             seed = np.random.randint(0,9,1)[0]
#             print('seed=',seed)
#             plt.figure()
#             plt.subplot(121)
...
#             plt.show()
            image_crop = np.pad(image,pad_width=((0,0),(75,75),(75,75)),mode='constant')
#             image = np.pad(image,pad_width=((0,0),(5,5),(5,5)),mode='constant')
            
            # print(image.shape)
            image = np.expand_dims(image,axis=1)
            image = torch.from_numpy(image)
            image_crop = np.expand_dims(image_crop,axis=1)
            ...
            # print(image.shape)
            mask_crop = np.pad(mask,pad_width=((0,0),(75,75),(75,75)),mode='constant')
#             mask = np.pad(mask,pad_width=((0,0),(5,5),(5,5)),mode='constant')
            # mask[mask == 255.] = 1
        ...
        if self.mode == 'train':    
            ...
            return img_file_name,image.float(),image_crop.float(),torch.from_numpy(np.expand_dims(mask, 1)).long(),torch.from_numpy(np.expand_dims(mask_crop, 1)).long(), coord, bbox
        if self.mode == 'valid':
