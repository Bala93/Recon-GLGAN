    ...
    def __getitem__(self, i):
        ...
        try:
            with h5py.File(fname, 'r') as data:
                #print(data.keys())
                input = np.pad(data['volus_2x'][:,:,slice], (5,5), 'constant', constant_values=(0,0))
                #input /= np.max(input)
                target_seg = np.pad(data['mask'][:,:,slice], (5,5), 'constant', constant_values=(0,0))
                target_rec = np.pad(data['volfs'][:,:,slice], (5,5), 'constant', constant_values=(0,0))
                ...
                '''
                return (torch.from_numpy(input), torch.from_numpy(target_rec), torch.from_numpy(target_seg))
        except Exception as e:
            print('here123')
            print(fname)
            print(e)
...
class SliceDataDev(Dataset):
    ...
    def __getitem__(self, i):
        ...
        try:
            with h5py.File(fname, 'r') as data:
                #print(data.keys())
                input = np.pad(data['volus_4x'][:,:,slice], (5,5), 'constant', constant_values=(0,0))
                #input /= np.max(input)
                target_seg = np.pad(data['mask'][:,:,slice], (5,5), 'constant', constant_values=(0,0))
                target_rec = np.pad(data['volfs'][:,:,slice], (5,5), 'constant', constant_values=(0,0))
                #coords = data['center_coord'][slice]
                ...
                #edgemap = np.pad(edgemap, (5,5), 'constant', constant_values=(0,0))
                return (torch.from_numpy(input), torch.from_numpy(target_rec), torch.from_numpy(target_seg), fname.name, slice)
                #coords = data['center_coord'][slice]
