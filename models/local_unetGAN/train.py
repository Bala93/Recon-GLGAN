...
from models.local_unetGAN.model import UNet, ContextDiscriminator
...
def train_epoch(args, epoch, model, netD, data_loader, optimizer, optimizerD, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        for i in range(input.shape[0]):
            ...
            cx = cx + 5
            cy = cy + 5
            o.append(outG[i,:,cy-size:cy+size,cx-size:cx+size]) 
        ...
        optimizer.zero_grad()
        pred_fake = netD(o_tensor, outG)
        loss_D_gan= criterion_bce(pred_fake, real_label)

        lossG_gan = (loss_D_gan)*scale
...
def visualize(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            input, target, coords, fname, slice = data
            input = input.unsqueeze(1).to(args.device)
            target = target.unsqueeze(1).to(args.device)
            ...
            save_image(o_tensor, 'ReconstructionPatch')
            save_image(torch.abs(t_tensor.float() - o_tensor.float()), 'PatchError')
            break
...
def build_disc():
    netD = ContextDiscriminator().to(args.device)
...
def create_arg_parser():
    parser = Args()
    parser.add_argument('--train-path', type=string, help='Path to training data')
    ...
    parser.add_argument('--roi-size', type=int, default=30, help='Dimensions of the square ROI')  
    parser.add_argument('--adv-weight', type=int, default=1e-4, help='Adversarial loss term weight')   
